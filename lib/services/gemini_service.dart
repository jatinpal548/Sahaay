...
class GeminiService {
  ...
  static const String _apiKey = String.fromEnvironment('GEMINI_API_KEY',
      defaultValue: 'AIzaSyB5JXU9UpCvyjN64KZsg33Wh9yqQAoMM5Q');

  // Fallback API keys (add more Gemini keys if you have them)
  static const List<String> _fallbackApiKeys = [
    'AIzaSyYOUR_SECOND_API_KEY_HERE',
    'AIzaSyYOUR_THIRD_API_KEY_HERE',
  ...
  static const Duration _timeout =
      Duration(seconds: 6); // Max 6 seconds timeout
  ...
  static Future<String> generateText(String prompt,
    ...
    if (_apiKey == 'YOUR_GEMINI_API_KEY_HERE' || _apiKey.isEmpty) {
      if (kDebugMode) {
        print('Gemini API key not configured. Using fallback response.');
      }
      return _generateFallbackResponse(
          prompt, Exception('API key not configured'));
    }
    ...
    for (final apiKey in _allApiKeys) {
      ...
      for (final model in _models) {
        ...
        for (int attempt = 0; attempt < maxRetries; attempt++) {
          try {
            final url =
                Uri.parse('$_basePath/$model:generateContent?key=$apiKey');
            ...
            if (response.statusCode == 200) {
              ...
              try {
                ...
                final errorMsg =
                    errorData['error']?['message'] ?? 'Model not found';
        ...
        if (!lastError.toString().contains('rate limit') &&
      ...
      if (lastError != null &&
          (lastError.toString().contains('rate limit') ||
              lastError.toString().contains('429'))) {
  ...
  static String _generateFallbackResponse(String prompt, Exception? error) {
    ...
    if (error != null &&
      ...
      if (lowerPrompt.contains('first message') ||
          lowerPrompt.contains('opening the chat')) {
        return "Hey! I'm Sahaay. How are you doing today? I'm here to listen whenever you need to talk.";
      }

      if (lowerPrompt.contains('stress level') ||
          lowerPrompt.contains('stressed')) {
      ...
      if (lowerPrompt.contains('anxious') ||
          lowerPrompt.contains('worry') ||
          lowerPrompt.contains('nervous')) {
      ...
      if (lowerPrompt.contains('sad') ||
          lowerPrompt.contains('down') ||
          lowerPrompt.contains('depressed')) {
    ...
    throw error ??
        Exception(
            'All API keys failed. Please check your internet connection and try again later.');
  }
}
