...
class _StressCheckScreenState extends State<StressCheckScreen>
  ...
  bool _canContinue = true; // Enable by default since slider has initial value
  ...
  void _navigateToHandler() {
    ...
    setState(() {
      _isNavigating = true;
      _canContinue = false; // Disable button during navigation
    });
    
    // Update stress level synchronously (level and profile name)
    final stressProvider = Provider.of<StressProvider>(context, listen: false);
    stressProvider.updateStressLevelSync(_stressLevel);
    
    // Trigger AI line generation in background (don't wait for it)
    stressProvider.updateStressLevel(_stressLevel).catchError((_) {
      // Silently handle errors - fallback message will be used
    });
                          ...
                          child: Opacity(
                            ...
                            child: Transform.scale(
                              ...
                              child: ElevatedButton(
                                onPressed: (_canContinue && !_isNavigating) ? _navigateToHandler : null,
